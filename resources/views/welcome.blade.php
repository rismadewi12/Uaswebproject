...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  ...
  <header id="header" class="fixed-top d-flex align-items-center">
    <div class="container d-flex align-items-center justify-content-between">
      ...
      <div >
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block bg-green-100">
                    @auth
                        <a href="{{ url('/dashboard') }}" class="p-1 mb-1 bg-info text-white"style="#464866">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="p-1 mb-1 bg-white text-green  border border-warning shadow-lg p-2 mb-3 bg-blue rounded" >Log in</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="p-1 mb-1 bg-white text-green  border border-warning shadow-lg p-2 mb-3 bg-blue rounded">Register</a>
                        @endif
                    @endauth
                </div>
            @endif
